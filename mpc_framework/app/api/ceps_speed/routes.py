...
def handle_protocol_random_share():
    if request.method == 'POST':
        ...
        config.preprocessing.protocol.handle_protocol_random_share(share, type)
    return "share"
...
def handle_protocol_double_random_share():
    if request.method == 'POST':
        ...
        type = request.form['type']
        config.protocol.preprocessing.handle_protocol_double_random_share(share_t, share_2t, type)
    return "share"
...
def handle_random_preprossing_shares():
    if request.method == 'POST':
        ...
        pdr_share_R = json.loads(request.form['pdr_share_R'])
        config.protocol.preprocessing.compute_preprossing_randomness(pr_share, pdr_share_r, pdr_share_R)
    return "share"
...
def handle_input_randomness():
    if request.method == 'POST':
        ...
        config.protocol.preprocessing.handle_random_input_shares(int(r), int(gid))
    return "share"
...
def handle_input_share():
    if request.method == 'POST':
        d = request.form['d']
        gid = request.form['gid']
        config.protocol.handle_input_share(int(d), int(gid))
    return "share"
